...
export default function AngryBirdsGame() {
    ...
    const createImpactEffect = (x: number, y: number, type: 'hit' | 'destroy' | 'bounce') => {
        ...
        setTimeout(() => {
            impactEffectsRef.current = impactEffectsRef.current.filter(e => e.id !== effect.id);
            setImpactEffects(prev => prev.filter(e => e.id !== effect.id));
        }, 1000);
    };

    const createElement = (type: 'img' | 'div', className: string, src?: string): HTMLElement => {
        if (type === 'img') {
    ...
    const createWorld = (engine: Matter.Engine) => {
        ...
        const slingshotLeftEl = createElement('div', 'w-2 h-20 bg-amber-800 rounded');
        const slingshotRightEl = createElement('div', 'w-2 h-20 bg-amber-800 rounded');
    ...
    const createStructures = (): GameElement[] => {
        ...
        const blockConfigs = [
            ...
            { x: 600, y: CANVAS_HEIGHT - 40, w: 120, h: 30, img: "/game/base/long_rectange_plank.png", density: 0.01 },
            { x: 720, y: CANVAS_HEIGHT - 40, w: 120, h: 30, img: "/game/base/long_rectange_plank.png", density: 0.01 },
            { x: 840, y: CANVAS_HEIGHT - 40, w: 120, h: 30, img: "/game/base/long_rectange_plank.png", density: 0.01 },

            // First level - very thick supports with high mass
            { x: 580, y: CANVAS_HEIGHT - 85, w: 30, h: 70, img: "/game/base/simple_rectangle_block.png", density: 0.008 },
            { x: 660, y: CANVAS_HEIGHT - 85, w: 30, h: 70, img: "/game/base/simple_rectangle_block.png", density: 0.008 },
            { x: 740, y: CANVAS_HEIGHT - 85, w: 30, h: 70, img: "/game/base/simple_rectangle_block.png", density: 0.008 },
            { x: 820, y: CANVAS_HEIGHT - 85, w: 30, h: 70, img: "/game/base/simple_rectangle_block.png", density: 0.008 },
            { x: 900, y: CANVAS_HEIGHT - 85, w: 30, h: 70, img: "/game/base/simple_rectangle_block.png", density: 0.008 },

            // First level platform - overlapping for stability
            { x: 650, y: CANVAS_HEIGHT - 125, w: 140, h: 25, img: "/game/base/long_rectange_plank.png", density: 0.006 },
            { x: 790, y: CANVAS_HEIGHT - 125, w: 140, h: 25, img: "/game/base/long_rectange_plank.png", density: 0.006 },

            // Second level - fewer, thicker supports
            { x: 680, y: CANVAS_HEIGHT - 170, w: 25, h: 70, img: "/game/base/simple_rectangle_block.png", density: 0.005 },
            { x: 760, y: CANVAS_HEIGHT - 170, w: 25, h: 70, img: "/game/base/simple_rectangle_block.png", density: 0.005 },

            // Top platform - smaller and lighter
            { x: 720, y: CANVAS_HEIGHT - 210, w: 80, h: 20, img: "/game/base/long_rectange_plank.png", density: 0.003 },
        ];

        blockConfigs.forEach(config => {
            const body = Matter.Bodies.rectangle(config.x, config.y, config.w, config.h, {
                label: "block",
    ...
    const createPigs = (): GameElement[] => {
        ...
        pigConfigs.forEach(config => {
            ...
            const element = createElement('img', 'w-8 h-8 pointer-events-none', "/game/pig.png");
            element.style.zIndex = '30'; // Ensure pigs are visible

            // Create health bar container (hidden)
            const healthBarContainer = createElement('div', 'absolute pointer-events-none');
            ...
            healthBar.style.backgroundColor = config.health === 3 ? '#ff4444' : '#ff6666';
    ...
    const createBird = () => {
        ...
        console.log('Creating bird type:', currentBirdType, 'Birds remaining:', gameState.birdsRemaining);
        ...
        const element = createElement('img', 'w-8 h-8 pointer-events-none cursor-grab', birdType.src);
        ...
        const gameElement: GameElement = { body, element, type: 'bird', birdType: currentBirdType };
    ...
    const startAnimationLoop = () => {
        const animate = () => {
            ...
            gameElementsRef.current = gameElementsRef.current.filter(gameElement => {
                if (gameElement.type === 'bird' && gameElement !== currentBirdRef.current) {
                    ...
                    const isWayOffScreen = gameElement.body.position.x > CANVAS_WIDTH + 300;
                    const isDeepUnderground = gameElement.body.position.y > CANVAS_HEIGHT + 200;
                    const isCompletelyStill = velocity < 0.02 && gameElement.body.position.y >= CANVAS_HEIGHT - 30;

                    if ((isCompletelyStill && timeSinceLaunch > 8000) || isWayOffScreen || isDeepUnderground) {
                        console.log('Removing stopped bird at:', gameElement.body.position.x, gameElement.body.position.y, 'velocity:', velocity, 'time since launch:', timeSinceLaunch);
    ...
    const handleCollisions = (event: Matter.IEventCollision<Matter.Engine>) => {
        event.pairs.forEach((pair) => {
            ...
            if (bodyA.label === "pig" || bodyB.label === "pig") {
                ...
                if (pigElement && engineRef.current && typeof pigElement.health === 'number') {
                    let damage = 0;
                    ...
                    if (damage > 0) {
                        ...
                        const timeSinceLastDamage = currentTime - (pigElement.lastDamageTime || 0);
                        ...
                        if (attackerBody.label === "bird" || timeSinceLastDamage > 500) {
                            ...
                            if (healthBar && pigElement.maxHealth) {
                                const healthPercent = (pigElement.health / pigElement.maxHealth) * 100;
                            ...
                            setTimeout(() => {
                                if (pigElement.element) {
                                    ...
                                    pigElement.element.style.transform = pigElement.element.style.transform.replace(/ scale\([^)]*\)/g, '');
                                }
                            }, 300);

                            // Remove pig if health reaches 0
                            if (pigElement.health <= 0) {
                                createImpactEffect(pigBody.position.x, pigBody.position.y, 'destroy');

                                Matter.World.remove(engineRef.current.world, pigBody);

                                pigElement.element.style.transition = 'all 0.5s ease-out';
                                pigElement.element.style.transform += ' scale(0) rotate(360deg)';
                                ...
                                const healthBarContainer = (pigElement as any).healthBarContainer;
                                ...
                                pigsRef.current = pigsRef.current.filter(p => p !== pigElement);
                                gameElementsRef.current = gameElementsRef.current.filter(g => g !== pigElement);
    ...
    const cleanup = () => {
        ...
        if (slingshotConstraintRef.current && engineRef.current) {
            Matter.World.remove(engineRef.current.world, slingshotConstraintRef.current);
        ...
        if (gameContainerRef.current && (gameContainerRef.current as any).slingshotElements) {
            (gameContainerRef.current as any).slingshotElements.forEach((slingshot: any) => {
        ...
        if (currentBirdRef.current && currentBirdRef.current.element && currentBirdRef.current.element.parentNode) {
        ...
        <div className="bg-blue-900/20 rounded-lg p-6 border-2 border-blue-500/50 mt-8 relative z-10">
            ...
            <div className="relative mx-auto border-2 border-blue-400/50 rounded-lg overflow-hidden w-full max-w-4xl">
                <div
                    ...
                    onMouseDown={(e) => {
                        ...
                        if (!currentBirdRef.current || !engineRef.current || !gameState.isRunning || isDragging) {
                            console.log('No bird or engine or game not running or already dragging');
                            return;
                        }
                        ...
                        const distance = Math.sqrt(
                            Math.pow(mouseX - bird.position.x, 2) +
                            Math.pow(mouseY - bird.position.y, 2)
                        );
                    ...
                    onMouseMove={(e) => {
                        if (!isDragging || !currentBirdRef.current || !engineRef.current) return;
                        ...
                        Matter.Body.setPosition(currentBirdRef.current.body, { x: newX, y: newY });
                    ...
                    onMouseUp={(e) => {
                        if (!isDragging || !currentBirdRef.current || !engineRef.current || !slingshotConstraintRef.current) return;
                        ...
                        console.log('Bird position before launch:', bird.position.x, bird.position.y);
                        ...
                        console.log('Bird velocity after launch:', bird.velocity.x, bird.velocity.y);
                        ...
                        setTimeout(() => {
                            setGameState(prev => {
                                ...
                                console.log('Birds remaining after this shot:', newBirdsRemaining);

                                if (newBirdsRemaining > 0) {
                                    setCurrentBirdType((prevType) => (prevType + 1) % BIRD_TYPES.length);
                                    createBird();
                                } else {
                                    checkGameEnd();
                                }
                ...
                >
                    ...
                    {!gameState.isRunning && !gameState.gameOver && (
                        <div className="absolute inset-0 flex items-center justify-center bg-black/70 z-40">
                            <div className="text-center text-white border-2 border-blue-400 bg-blue-900/50 p-4 rounded">
                                ...
                                <p className="text-sm">Drag the bird back and release to launch at the pigs</p>
                            </div>
                        </div>
                    )}
                    ...
                    {impactEffects.map(effect => (
                        <div
                            key={effect.id}
                            className={`absolute pointer-events-none z-50 ${effect.type === 'hit' ? 'animate-ping' :
                                effect.type === 'destroy' ? 'animate-bounce' : 'animate-pulse'
                                }`}
                            style={{
                                left: `${(effect.x / CANVAS_WIDTH) * 100}%`,
                                top: `${(effect.y / CANVAS_HEIGHT) * 100}%`,
                                transform: 'translate(-50%, -50%)',
                                animation: effect.type === 'destroy' ?
            ...
            <div className="text-center mt-4">
                <p className="text-sm text-blue-200">
                    Drag the bird back in the slingshot and release to launch it at the green pigs!
                </p>
